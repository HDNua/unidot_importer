...
| --- | --- | --- | :---: | --- |
| Synty Studios | POLYGON - Prototype Pack | Godot `4.7.1-stable.mono`, macOS | △ 부분 지원 | 패키지의 외형과 배치를 결정하는 요소는 전부 변환되고 검증되었습니다. 셰이더 시맨틱과 ParticleSystem의 고급 절반은 변환되지 않습니다. 영역별 상세는 아래에 있습니다. |

`△ 부분 지원`은 검증된 사용 가능 부분집합이 존재하지만 임포트가 무손실은 아니며,
위에 나열된 격차에 대해서는 검토나 수동 이식이 필요하다는 뜻입니다. 상태가 `△`로
남는 이유는 ShaderGraph 콘텐츠가 아예 번역되지 않기 때문이지, 변환된 지오메트리가
미덥지 못해서가 아닙니다.

| 영역 | 상태 | 비고 |
| --- | :---: | --- |
| 메시·트랜스폼·프리팹/씬 계층 | ○ 변환됨 | 프리팹·씬 `498/498` 인스턴스화, 생성된 씬 리소스 `989/989` 로드 |
| 머티리얼과 albedo 텍스처 | ○ 변환됨 | 변환된 머티리얼 `139`개 중 `121`개에 텍스처 연결. 나머지 `18`개는 Unity 원본에도 텍스처가 없음 |
| 콜리전 셰이프 | ○ 변환됨 | 소스 메시가 패키지에서 누락된 콜라이더 `13`개 제외 |
| GameObject active 상태와 렌더러 가시성 | ○ 변환됨 | 가시 `8` / 숨김 `24`, 불일치 `0` |
| 휴머노이드 리그와 스키닝 | ○ 변환됨 | 스킨 변형 검사 `3,600`건, 실패 `0`건 — [휴머노이드 스키닝 정확도](#휴머노이드-스키닝-정확도) 참조 |
| 씬 루트 순서 | ○ 변환됨 | 데모 씬의 저작 루트 순서 복원 |
| 라이트맵 저작 값 | ○ 변환됨 | 바운스·모드·texel 스케일·텍스처 크기 보존 |
| ParticleSystem | △ 부분 | 결정적 공통 부분집합만 변환. 생략·근사된 모듈은 경고 `108`건으로 표시 |
| 리얼타임 GI | ✗ 미변환 | Godot `LightmapGI`에 대응되는 리얼타임 기능이 없어 Unity의 의도는 메타데이터로만 유지 |
| ShaderGraph / SubGraph | ✗ 미변환 | `25`개 전부 수동 이식용 소스로 보존. 시맨틱은 번역되지 않음 |

임포트 중 출력되는 모든 진단 메시지는
[임포트 진단 메시지 해설](#임포트-진단-메시지-해설)에서 빠짐없이 설명하며, 남은 결함은
[알려진 한계](#알려진-한계)에 정리되어 있습니다.
...
*이 패키지를 정상적으로 임포트했을 때의 화면입니다. `Import complete.`와 함께 경고
`124`건, 오류 `13`건이 카운터에 남아 있습니다. 이 절은 그 전부를 하나도 빠짐없이
설명합니다.*
...
진단 중 하나는 Unidot이 고칠 수 없는 실제 결함입니다. Godot의 FBX 임포터는 FBX에
내장된 텍스처 참조를 후보 디렉터리를 훑어가며 해석하는데, 그 후보 중에 소문자
`textures/`가 있습니다. 대소문자를 구분하지 않는 파일시스템에서는 이 탐색이
`Textures/`에 저장된 파일을 열어버리고, Godot이 대소문자 불일치 경고를 낸 뒤 요청한
표기가 그대로 추출 메시에 기록됩니다. 이 패키지에서는 텍스처 하나
(`Generic_Road_01.png`)가 `Models/extracted/` 아래 `23`개 파일에 걸쳐 해당되며, 대소문자를
구분하는 파일시스템으로 익스포트하면 그 참조가 로드되지 않습니다. 변환된 `.mat`
리소스는 영향이 없으므로 프리팹과 씬은 정상 렌더링됩니다. Unidot은 post-import
스크립트에서 이 참조를 고칠 수 없습니다 — 해당 `.mesh` 파일을 Unidot이 쓰긴 하지만,
그 뒤에 Godot 자신의 `save_to_file` 서브리소스 추출이 같은 경로를 다시 쓰기 때문에
post-import에서 한 수정은 덮어써집니다. 제대로 고치려면 엔진의 텍스처 탐색 규칙이나
모델 `.import` 설정의 메시 추출 방식을 바꿔야 합니다.
